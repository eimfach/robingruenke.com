...
def journalcontent(doc, data, features):
    ...
    if features["chapter-index"] and len(data.items) > 2:
        ...
        html.components.chapterindex(doc, data.items, ids=ids)

    doc.line("div", "", klass="pagebreak")

    for chapter in data.items:
        html_id = getnormalizedtopic(chapter.topic)
        html.components.chapter(doc, html_id, chapter, features)
    ...
    if features["journal-like"]:
        html.components.like(doc, data.meta.title)
