        ...
        if picture:
            ...
            with doc.tag('div', klass=containerClasses):
                doc.stag('img', loading='lazy', klass='main-image',
                         src=picture['src'], style='display: block; max-height: ' + picture['height'])

                if gallery:
                    segmentedPictures = numpy.array(
                        gallery['pictures']).reshape(-1, 3)

                    for gallerysegment in segmentedPictures:
                        with doc.tag('div', klass='gallery-container'):
                            for gallerypicture in gallerysegment:
                                doc.stag('img', loading='lazy', klass='gallery-picture',
                                         src=gallerypicture, style='max-height: ' + gallery['height'])
...
def intro(doc, text):
    ...
    with doc.tag('blockquote', klass='margin-bottom-20', id='intro-text'):
        ...
        if md['link'] is not None:
            doc.line('a', md['link']['description'],
                     href=md['link']['url'], target='_blank')
